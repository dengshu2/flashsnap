...
8. <body> 内必须直接包含一个有实际内容的 \`.card\` 根节点，不能只输出背景、样式或空容器
9. 禁止使用 \`100vh\`、\`100dvh\`、\`height: 100%\`、\`min-height: 100vh\`、\`position: fixed\` 等依赖视口高度的页面级布局
10. 不要写全页海报式空白背景；所有视觉设计都应服务于卡片正文，正文内容必须真实存在且可见
11. 保持 CSS 精炼，避免超长 reset、重复工具类或大段无意义装饰代码
`;
